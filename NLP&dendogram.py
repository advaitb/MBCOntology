...
for i in range(243):
    filtered = []
    with open(path+'Community_no_'+str(i)+'.txt','r') as f:
        lines = f.readlines()
        for line in lines:
            if "<AbstractText>" in line:
                ...
                filtered_words = [lemma.lemmatize(word.lower()) for word in tokens if word not in stopwords.words('english') and word not in extras and not is_float(word)]
                #tokens.remove('.')
                #tokens.remove(',')
                filtered = filtered + filtered_words
        final_text = ' '.join(list(set(filtered)))
    with open(write_path+'Community_no_'+str(i)+'.txt','w') as w:
        w.write(final_text)
